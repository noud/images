                ...
                <tr>
                    <td class="formborder">
                        ...
                        @foreach($data->collection as $image)
                            <img src="/uploads/avatar/{{$image->uploaded_at->format('Y')}}/{{$image->uploaded_at->format('m')}}/{{$image->uploaded_at->format('d')}}/{{$image->filename}}.{{$image->image_type ? $image->image_type->name : 'gif'}}"
