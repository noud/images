...
$(document).ready(function () {
  $('body').on('click', '.emoticon', function () {
    // if(window.opener.$!==undefined&&window.opener.$.mydefchatconn!==undefined){
    //   window.opener.$.mydefchatconn("select_emoticon",$(this).data('emoticon'));
    // }else{
    // window.opener.use_uploaded_emoticon($(this).data('emoticon'));
    // }
    const text = $(this).data('emoticon');
    const el = $('#chat_input')
    var val = el.val();
    var position = val.slice(0, el[0].selectionStart).length;
    var output = [val.slice(0, position), text, val.slice(position)].join('');
    console.log(output);
    $('#chat_input').val(output);
    // window.opener.focus();
    // window.close();
  });
  ...
  $('body').on('mousedown', 'a[rel*=facebox]', function () {
    $(this).facebox();
  });
});;
...
$.facebox.settings.closeImage = '/images/closebutton.png';
